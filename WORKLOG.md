...
- User installed the debug APK on a phone and reported that it works. The exact GPS, speech, reward, and restart actions tried on that phone are not yet known. A separate local emulator force-stop/relaunch check retained discovery, 10 XP, and 10 stars and resumed from Home with the quest available; this covers early progress persistence but not the complete reward flow.
...
4. Online street map is integrated and has rendered successfully on the local Pixel 2 API 35 emulator. Optional downloadable offline packs remain future work; map boundaries and the 200 MB budget are recorded in `docs/LOCATION.md`. Exercise permission denied, approximate location, disabled GPS, and stale/unavailable fixes on real devices.
...
1. **Finish the MVP acceptance path.** On the phone, use a test POI at the current location to complete check-in, quest, Spanish word, and quiz. Use Developer Mode to supply the remaining stars for the 100-star ice-cream reward: the single sample place currently pays 50 stars in total. Request the reward, approve it with the parent PIN, force-close and reopen the app, and confirm the badge, XP, remaining stars, reward history, and one-time payout. Record specific failures and fix them before adding content.
2. **Check location and speech failure paths on the phone.** Exercise precise and approximate location, permission denial, disabled location services, an unavailable/poor fix, moving outside and back into a test POI, and Spanish TTS. The emulator proves the foreground request path, but it cannot prove real-world accuracy or audio. Keep background geofencing and notifications optional because foreground check-in is the MVP requirement.
3. **Prepare a stable update path before wider installation.** Current APKs are debug-signed, so APKs built on another computer may have a different signing certificate and cannot update an existing install in place. Choose one private release keystore and store it outside Git; document secure build/backup and a transition plan for phones already running the debug APK. Protect local progress during any uninstall/reinstall or device transfer.
4. **Complete the presentation layer.** Replace emoji placeholders with reusable avatar poses and distinct celebration, badge, level-up, and reward animations; support reduced motion and check small-screen layout. Keep game rewards independent of animation completion.
5. **Expand content only after the acceptance path passes.** Add Barcelona places through the City Pack schema. Online street maps are sufficient for the current use case; an offline pack is optional and needs a licensed data source plus a measured size at or below the 200 MB budget.
...
Roughly 33% of the full project; the core MVP game-engine flow is largely implemented. The ordered engine, persistence, GPS rules, parent-gated reward accounting and approval, device-local PIN, diagnostics, test POIs, anti-farming per-POI reset, Spanish TTS, first-run onboarding, online street map integration, typed observation/say-phrase/multiple-choice/parent-check quests, level threshold feedback, and reusable manga-burst celebration are implemented. The local Pixel 2 API 35 instrumentation suite passes 4/4 and the map renders online street tiles. Physical GPS/TTS validation, parent-flow device check, force-stop process-death check, optional licensed offline map pack, richer avatar/confetti/points animations, and production privacy/UX review remain. Unit tests, instrumentation compilation, and debug APK assembly pass locally. Re-estimate after each major verified milestone.
