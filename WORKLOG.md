...
2. Add a progress export/import or migration mechanism before moving the current debug-signed phone install to a release-signed APK. Release signing support is in Gradle, but no private keystore has been created.
...
2. **Prepare a stable update path before wider installation.** Gradle now supports a private release keystore supplied through environment variables, and release packaging stops if signing data or the keystore is missing. Generate and safely back up the keystore before the first release build. The current phone has a debug-signed install; build a progress export/import or migration path before switching signatures.
...
Roughly 35% of the full project; the core MVP flow has passed the phone checklist according to the user. The ordered engine, persistence, foreground GPS check-in, PIN-approved reward flow, diagnostics, test POIs, Spanish TTS, online street map, data-driven quests, and basic celebrations are implemented. Emulator instrumentation passes 4/4; the user confirms GPS, speech, quiz retry, reward approval, and restart on the phone. Remaining work: GPS permission/error edge cases, progress-safe migration to a private release signing key, richer reusable animations and accessibility, optional licensed offline maps, more City Pack places, and final privacy/UX review. Debug APK assembly succeeds. Release signing configuration is present; no keystore or passwords are stored in the repository.
