...
GitHub Actions runs the unit tests, compiles the instrumentation test APK, and builds a debug APK. Hosted runners in the attempted configurations could not boot this emulator, so CI does not claim that the smoke test executed. Run the instrumentation test from Android Studio or on a local emulator/device.
